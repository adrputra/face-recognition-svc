...
import (
	"context"
	"github.com/adrputra/face-recognition-svc/gateway-gin/app/client"
...
func (c *InstitutionController) InsertNewInstitution(ctx context.Context, institution *model.Institution) error {
	...
	_, err := utils.GetMetadata(ctx)
	if err != nil {
		utils.LogEventError(span, err)
		return err
	}
	institution.ID = uuid.New().String()
	institution.CreatedAt = utils.LocalTime()
	institution.UpdatedAt = utils.LocalTime()
