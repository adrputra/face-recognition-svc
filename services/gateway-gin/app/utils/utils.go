...
func JSON(c *gin.Context, code int, payload any) error {
	c.JSON(code, payload)
	return nil
}

func LogError(c *gin.Context, err error, stack []byte) error {
	log.Error().Err(err).Bytes("stack", stack).Msg("Error occurred")
	data, ok := err.(*model.ErrorResponse)
	if ok {
		c.JSON(data.Code, model.Response{
			Code:    data.Code,
			Message: err.Error(),
			Data:    nil,
		})
...
// ParseFilterFromQuery parses filter parameters from request query parameters.
func ParseFilterFromQuery(c *gin.Context) *model.Filter {
...
// ParsePaginationFromQuery parses pagination parameters from request query parameters.
func ParsePaginationFromQuery(c *gin.Context) *model.Pagination {
